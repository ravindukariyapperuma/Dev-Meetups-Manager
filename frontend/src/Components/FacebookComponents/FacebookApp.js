...
function FacebookApp() {
  ...
  <FBDrawer />
    ...
    <div className="jumbotron"  style={{ width: "70%",marginLeft:"25%",marginTop:"-2%", backgroundColor:"#EBF4FA"}}>

      <button onClick={logout} type="submit" value="Submit" style={{width: "10%", marginLeft: "90%"}} className="btn btn-primary">
        Logout
      </button>
    ...
    <div className="card-body">
      ...
      <h5 className="card-title" style={{marginLeft: "30%", fontSize:25, marginTop:"2%", fontFamily:'Roboto'}}>Welcome { user && user.name}!</h5><br/>
      <img src={fblogo} height={100} width={100} style={{marginLeft: "1%", marginTop:"-10%"}}/>

        <br/><br/>
      <h5 className="card-title" style={{marginLeft: "1%", fontSize:20, fontFamily:'Roboto'}}>Create your facebook announcement</h5><br/>
      ...
      <Row>
        {posts && posts.map(product => (
            <Posts key={product.id} product={product} name={page && page[0].name} deleteData={deleteData} setDeleteData={setDeleteData}/>
        ))}
      </Row>
  </div>;
}
